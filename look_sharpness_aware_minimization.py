...
# ==============================================================================
"""Towards Efficient and Scalable Sharpness-Aware Minimization implementation."""
...
class LookSharpnessAwareMinimization(Model):
    """Towards Efficient and Scalable Sharpness-Aware Minimization (LookSAM) training flow.
    ...
    Args:
      ...
      k: int, defaults to 10.  frequency of SAM's gradient calculation
      ...
      num_batch_splits: int, defaults to None. The number of mini batches to
        ...
        sub-batches.
      name: string, defaults to None. The name of the LookSAM model.
    Reference:
      [Sharpness-Aware Minimization for Efficiently Improving Generalization, 2020](https://arxiv.org/abs/2010.01412)
      [Towards Efficient and Scalable Sharpness-Aware Minimization, 2022](https://arxiv.org/abs/2203.02714)
    """

    def __init__(self, model, rho=0.05, k=10, num_batch_splits=None, name=None):
    ...
    def train_step(self, data):
        """The logic of one LookSAM training step.
        Args:
        ...
        def with_sam_fn():
            ...
            return tf.concat(pred_all_batches, axis=0)
        ...
        pred = tf.cond(tf.equal(self.optimizer.iterations + 1 % self.k, 0), with_sam_fn, without_sam_fn)
        self.compiled_metrics.update_state(y, pred, sample_weight)
        return {m.name: m.result() for m in self.metrics}

    def call(self, inputs):
        """Forward pass of LookSAM.
        LookSAM delegates the forward pass call to the wrapped model.
        Args:
