...
"""Surrogate Gap Sharpness Aware Minimization implementation.
   ...
   Reference:
     ...
     [2] https://github.com/google-research/big_vision/blob/main/big_vision/trainers/proj/gsam/gsam.py
"""
    ...
    def train_step(self, data):
        ...
        if self.lr_max == self.lr_min:
            ...
            rho = self.rho_min + (self.rho_max - self.rho_min) * (
                    self.optimizer._decayed_lr(tf.float32) - self.lr_min) / (self.lr_max - self.lr_min)
        ...
        for x_batch, y_batch in zip(x_split, y_split):
            ...
            scale = rho / (gradients_order2_norm + self.epsilon)
            ...
            g_clean_residual = [(a - g_clean_projection_norm * b) for a, b in zip(gradients, g_robust_normalized)]
            ...
            if len(gradients_all_batches) == 0:
                ...
                for gradient, gradient_all_batches in zip(g_gsam, gradients_all_batches):
